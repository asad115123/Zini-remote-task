...
class _GetAllMesgState extends State<GetAllMesg> {
  ...
  String formatTime(String time) {
    try {
      DateTime parsedTime =
          DateTime.parse(time); // Parse the time string to DateTime
      return DateFormat('yyyy/MM/dd')
          .format(parsedTime); // Format the DateTime object
    } catch (e) {
            ...
            children: [
              ...
              provider.loading
                  ...
                  : provider.messages.isEmpty
                      ? Center(
                          child:
                              Text('No messages found')) // Show if no messages
                      : ListView.builder(
                          physics: NeverScrollableScrollPhysics(),
                          shrinkWrap: true,
                          itemCount:
                              provider.messages.length, // Number of messages
                          itemBuilder: (context, index) {
                            var messageData =
                                provider.messages[index]; // Access each message
                            return Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 5),
                                  Text(
                                      'From: ${messageData['from'] ?? 'Unknown'}',style: TextStyle(fontWeight: FontWeight.w500,fontSize: 14),), // Display 'from'
                                  SizedBox(height: 5),
                                  Text(
                                      'Time: ${formatTime(messageData['time'])}',style: TextStyle(fontWeight: FontWeight.w500,fontSize: 14),), // Format and display time
                                ],
                              ),
                            );
                          },
